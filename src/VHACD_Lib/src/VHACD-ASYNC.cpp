...
class MyHACD_API : public VHACD::IVHACD,
 ...
 public:
  ...
  virtual bool Compute(const double* const _points,
    ...
    // we can operate on them safely from the background thread.
  ...
  bool ComputeNow(const double* const points,
    ...
    if (countPoints) {
      bool ok =
          mVHACD->Compute(points, countPoints, triangles, countTriangles, desc);
      if (ok) {
        ...
        for (uint32_t i = 0; i < ret; i++) {
          ...
          h.m_triangles =
              (uint32_t*)HACD_ALLOC(sizeof(uint32_t) * 3 * h.m_nTriangles);
          memcpy(h.m_triangles, vhull.m_triangles,
                 sizeof(uint32_t) * 3 * h.m_nTriangles);
  ...
  virtual void GetConvexHull(const uint32_t index,
                             VHACD::IVHACD::ConvexHull& ch) const final {
    if (index < mHullCount) {
      ch = mHulls[index];
    }
  }
  ...
  void releaseHACD(
      void)  // release memory associated with the last HACD request
  {
  ...
  virtual void Cancel() final {
    ...
    if (mThread) {
      mThread->join();  // Wait for the thread to fully exit before we delete
                        // the instance
      delete mThread;
      mThread = nullptr;
  ...
  virtual bool Compute(const float* const points,
    ...
    bool ret =
        Compute(vertices, countPoints, triangles, countTriangles, params);
    HACD_FREE(vertices);
    return ret;
  }
  ...
  void processPendingMessages(void) const {
    ...
    if (mHaveUpdateMessage && mCallback) {
      mMessageMutex.lock();
      mCallback->Update(mOverallProgress, mStageProgress, mOperationProgress,
                        mStage.c_str(), mOperation.c_str());

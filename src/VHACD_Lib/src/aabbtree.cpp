...
private:
	...
	bool GetClosestPointWithinDistance(const Vec3& point, const double maxDis, double& dis, double& v, double& w, uint32_t& faceIndex,Vec3 &closest) const;

	virtual bool getClosestPointWithinDistance(const double *point,double maxDistance,double *closestPoint) final
	{
		double dis,v,w;
		uint32_t faceIndex;
		bool hit = GetClosestPointWithinDistance(*(const Vec3 *)point,maxDistance,dis,v,w,faceIndex,*(Vec3 *)closestPoint);
		return hit;
	}

	void GetClosestPointWithinDistanceSqRecursive(uint32_t nodeIndex, const Vec3& point, double& outDisSq, double& outV, double& outW, uint32_t& outFaceIndex,Vec3 &closest) const;
...
bool AABBTreeImpl::TraceRay(
    const Vec3& start, const Vector3& dir, double& outT, double& u, double& v, double& w, double& faceSign, uint32_t& faceIndex) const
{
    outT = FLT_MAX;
...
bool AABBTreeImpl::GetClosestPointWithinDistance(const Vec3& point, const double maxDis, double& dis, double& v, double& w, uint32_t& faceIndex,Vec3 &closest) const
{
    dis = maxDis;
...
void AABBTreeImpl::GetClosestPointWithinDistanceSqRecursive(uint32_t nodeIndex, const Vec3& point, double& outDisSq, double& outV, double& outW, uint32_t& outFaceIndex,Vec3 &closestPoint) const
{
    const Node& node = m_nodes[nodeIndex];
    ...
    if (node.m_faces == NULL)
    {
        ...
        if (dcSq < outDisSq)
		{
            GetClosestPointWithinDistanceSqRecursive(node.m_children + closest, point, outDisSq, outV, outW, outFaceIndex,closestPoint);
		}

        if (dfSq < outDisSq)
		{
            GetClosestPointWithinDistanceSqRecursive(node.m_children + furthest, point, outDisSq, outV, outW, outFaceIndex,closestPoint);
		}
    }
    else
    {
